...
class HuxiuFetcher(BaseFetcher):
    ...
    使用 RSS: https://www.huxiu.com/rss/0.xml
    """
    ...
    def fetch(self, limit: int = 20) -> List[NewsItem]:
        ...
        try:
            ...
            logger.info(f"Fetched {len(items)} items from 虎嗅")
            return items

        except Exception as e:
            logger.error(f"Failed to fetch 虎嗅: {e}")
            return []
    ...
    def _parse_entry(self, entry) -> NewsItem:
        ...
        try:
            ...
            logger.warning(f"Failed to parse 虎嗅 entry: {e}")
            return None
