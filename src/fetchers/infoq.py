...
class InfoQFetcher(BaseFetcher):
    """InfoQ 抓取器

    使用 RSS: https://www.infoq.cn/feed
    """
    ...
    def fetch(self, limit: int = 20) -> List[NewsItem]:
        ...
        try:
            ...
            logger.info(f"Fetched {len(items)} items from InfoQ")
            return items

        except Exception as e:
            logger.error(f"Failed to fetch InfoQ: {e}")
            return []
    ...
    def _parse_entry(self, entry) -> NewsItem:
        ...
        try:
            ...
            logger.warning(f"Failed to parse InfoQ entry: {e}")
            return None
